	...
	function create_field( $field )
	{
		...
		if( is_array($field['choices']) )
		{
			foreach( $field['choices'] as $key => $value )
			{
				...
				if( strtolower(esc_attr($field['id'])) == 'acf-field-layout')
				{
					$e .= '<li>';
					$e .= '<div class="img-admin-layout-'.$i.'"></div>';	
				}
				$e .= '<label><input id="' . esc_attr($field['id']) . '-' . esc_attr($key) . '" type="radio" name="' . esc_attr($field['name']) . '" value="' . esc_attr($key) . '" ' . esc_attr( $atts ) . ' />' . $value . '</label></li>';
			}
		}
		...
		if( $field['other_choice'] )
		{
			...
			$e .= '<label><input id="' . esc_attr($field['id']) . '-other" type="radio" name="' . esc_attr($field['name']) . '" value="other" ' . $atts . ' />' . __("Other", 'acf') . '</label> <input type="text" ' . $atts2 . ' /></li>';
		}
